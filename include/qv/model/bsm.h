...
_BEGIN_NAMESPACE_MODEL
  ...
  public:
    ...
    struct compute
    {
      ...
      )
      {
        _QV_STATIC_CONSTEXPR auto half { 0.5 };
         return (log(S / K)+ (r + half * pow2(sigma)) * T) 
            / (sigma * sqrt(T)); 
      }
      ...
      _QV_CONSTEXPR_SCOPE
      static std::pair<_R, _R> call_put(
        const _R S, 
        const _R K, 
        const _R T, 
        const _R r, 
    ...
    _QV_CONSTEXPR_SCOPE
    _R d1() const noexcept
    { return compute::d1(S(), K(), T(), r(), sigma()); } 
    

    _QV_CONSTEXPR_SCOPE
    _R d2() const noexcept
    { return compute::d2(S(), K(), T(), r(), sigma()); }
    ...
    _QV_CONSTEXPR_SCOPE
    _R put() const noexcept
    { return compute::put(S(), K(), T(), r(), sigma()); }
  ...
  {    
   ...
   public:
     ...
     data_impl(
			...
			const _R sigma,
      const _R q
		) noexcept : base(S, K, T, r, sigma), _q(q)
	  {}
    ...
    _QV_CONSTEXPR_SCOPE
    void set_q(const _R q) noexcept
    { _q = q; }
   ...
   public:
    ...
    bsm(
      ...
      const _R q
		...
		noexcept : _model(S, K, T, r, sigma, q)
    {}
    
    _QV_CONSTEXPR_SCOPE _R 
		d1() const noexcept
    { 
      _QV_STATIC_CONSTEXPR auto half { 0.5 };
