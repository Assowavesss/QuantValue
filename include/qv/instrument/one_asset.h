...
class one_asset : public option<_M, _Ex>
{
public:
  ...
  _QV_CONSTEXPR_SCOPE one_asset(const _T&... args) noexcept : base(args...)
  {
  }

  _QV_CONSTEXPR_SCOPE
  explicit one_asset(const _M& model) noexcept : base(model) {}

  _QV_CONSTEXPR_SCOPE
  explicit one_asset(_M&& model) noexcept : base(std::forward<_M>(model)) {}
