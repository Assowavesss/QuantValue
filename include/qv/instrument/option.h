...
_BEGIN_NAMESPACE_INSTRUMENT
  ...
  class option : 
    public greeks<_M, _Ex>,
    public qv::detail::crtp<option<_M, _Ex>>
  {
  public:
  ...
  protected:
   ...
   template<typename ..._T> 
    _QV_CONSTEXPR_SCOPE
    option(const _T&... args) noexcept :
      greeks_adaptor::greeks(args...)
    {}
    
    _QV_CONSTEXPR_SCOPE
		explicit option(const _M& model) noexcept :
		  greeks_adaptor::greeks(model) 
		{}
  ...
  public:
    ...
    _QV_CONSTEXPR_SCOPE
    data_type payoff() const noexcept
    { return base::impl().payoff(); }
