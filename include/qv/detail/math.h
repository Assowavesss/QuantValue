...
constexpr auto sqrt2() noexcept { return std::sqrt(2); }
...
_QV_CONSTEXPR auto pow(const _R base, const _I exp) noexcept
{
...
template<real _R>
_QV_CONSTEXPR auto pow2(const _R base) noexcept
{
  return base * base;
}

template<real _R>
_QV_CONSTEXPR auto pow3(const _R base) noexcept
{
  return base * pow2(base);
}

template<real _R, auto real_zero = _R(0)>
_QV_CONSTEXPR auto sqrt(const _R num) noexcept
{
  if (num < real_zero)
    limit<_R>::quiet_NaN();
...
template<real _R>
_QV_CONSTEXPR auto erf(const _R num) noexcept
{
  return std::erf(num);
}

template<real _R, auto real_zero = _R(0)>
_QV_CONSTEXPR _R log(const _R num)
{
  if (num == real_zero)
    return limit<_R>::min();
...
template<real _R>
_QV_CONSTEXPR auto exp(const _R num) noexcept
{
  return std::exp(num);
}

template<real _R, auto half = 0.5, auto one = 1, auto sqrt_two = sqrt2()>
_QV_CONSTEXPR auto
cnd(const _R x, const _R mean = 0.0, const _R stddev = 1.0) noexcept
{
  return half * (one + erf((x - mean) / (stddev * sqrt_two)));
}

template<real _R, auto half = 0.5, auto one = 1, auto two = 2>
_QV_CONSTEXPR _R pdf(const _R x) noexcept
{
  return (one / sqrt(two * M_PI)) * exp(-half * pow2(x));
}

template<real _R, auto half = 0.5, auto sqrt_two = sqrt2()>
_QV_CONSTEXPR _R normal_cdf(const _R x) noexcept
{
...
template<typename _R, typename = is_convertible_real<_R>>
_QV_CONSTEXPR _R log(const _R num) noexcept
{
